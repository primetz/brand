...
const cart = {
    ...
    methods: {
        addProduct(product) {
            const find = this.products.find(el => el.id_product === product.id_product);
            if (find) {
        ...
        removeProduct(product) {
            ...
            if (find && find.quantity > 1) {
                ...
                this.$root.deleteJson(`/api/cart/${find.id_product}`, product)
                    .then(data => {
                        if (data.result === 1) {
        ...
        getTotal() {
            return {
                price: this.products.reduce((sum, item) => sum + item.price * item.quantity, 0),
                quantity: this.products.reduce((sum, item) => sum + item.quantity, 0)
            };
        }
    },
    mounted() {
    ...
    template: `<div class="cart-menu-wrp">
